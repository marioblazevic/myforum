<div class="mb-3">
    <span class="like">
        <form class="inline-block" action="{{ route('like', $post) }}" method="post">
            @csrf
            <button type="submit">
                <svg viewBox="0 0 20 20" class="text-gray-500 w-4 mr-1 inline-block {{ $post->isLikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
    ...
    <span class="dislike">
        <form class="inline-block" action="{{ route('dislike', $post) }}" method="post">
                @csrf
                @method('DELETE')
                <button type="submit">
                    <svg viewBox="0 0 20 20" class="text-gray-500 w-4 mr-1 inline-block {{ $post->isDislikedBy(auth()->user()) ? 'text-blue-500' : 'text-gray-500' }}">
                        <g id="Page-1" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                            <g id="icon-shape" class="fill-current">
